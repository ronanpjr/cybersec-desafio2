# Registro de planejamento — Lab: Exploiting NoSQL operator injection to extract unknown fields
...
> **Documento histórico de decisões.** O guia canônico para a versão atual é o [README da raiz](../README.md). Este registro foi atualizado nos pontos que descrevem caminhos e comportamentos vigentes, mas deve ser lido como histórico de planejamento, não como roteiro de execução.
...
- `POST /forgot-password` (`username` → solicita/renova token; token + senhas → troca senha e limpa token).

**Seed** — container init que insere `wiener` (login normal) e `carlos` (com `passwordReset` aleatório de
16 caracteres hex). O exploit descobre o campo e o valor de forma independente.
...
5. `GET /forgot-password?campo=token` → extrai inputs do form (CSRF incluso, se houver) → `POST /forgot-password`
...
1. Nome do campo fixo (`passwordReset`) favorece reprodutibilidade; o lab real usa nome aleatório por
...
- Versão corrigida rejeita `$ne`/`$where` com mensagem genérica em HTTP 200 e bloqueia a exfiltração.
- `.gitignore` cobre `.env`, dados do Mongo, builds, `node_modules`, `__pycache__`, logs, artefatos de seed.
- `documentacao/docs/LAB_LIVE.md` registra a execução real (token efêmero redigido no commit).
...
- **Lab ao vivo resolvido**: campo `passwordReset`, token exfiltrado, senha redefinida, login como
  `carlos`, banner **Solved**. Registro em `documentacao/docs/LAB_LIVE.md`.
...
- **Documentação**: `README.md`, `documentacao/docs/WRITEUP.md`, `documentacao/docs/LAB_LIVE.md`, `documentacao/docs/PREVENTION.md`.
